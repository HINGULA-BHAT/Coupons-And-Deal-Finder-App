...
public class CustomerService  {
	...
	public ArrayList<Coupon> getCoupons() throws CustomException {
		if(couponsRepo.findAll().isEmpty()) {
			throw new CustomException("There are no coupons in the system ");
		}
		return (ArrayList<Coupon>)couponsRepo.findAll();
	}
					...
					if(existingCoupon.getOffer() > 0) {
						if(existingCoupon.getEndDate() != null && existingCoupon.getEndDate().getTime() <= Calendar.getInstance().getTime().getTime())
							...
							if(customerOptional.isPresent()){
								...
								existingCoupon.setOffer(existingCoupon.getOffer() - 1);
	...
	public ArrayList<Coupon> getCustomerCoupons(double maxPrice, String customerId) throws CustomException {
		if(maxPrice > 0) {
			...
			if(optionalCustomer.isPresent()){
				Customer customer = optionalCustomer.get();
				ArrayList<Coupon> list = customer.getCoupons().stream().filter(coupon ->
						coupon.getOffer()<=maxPrice).collect(Collectors.toCollection(ArrayList::new));
				if(list.isEmpty()){
